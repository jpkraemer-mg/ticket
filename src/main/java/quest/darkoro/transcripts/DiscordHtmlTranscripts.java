...
import quest.darkoro.transcripts.utils.format.impl.AudioFormat;
import quest.darkoro.transcripts.utils.format.impl.ImageFormat;
import quest.darkoro.transcripts.utils.format.impl.VideoFormat;
...
public class DiscordHtmlTranscripts {
  ...
  private void handleEmbedImage(Document document, MessageEmbed embed,
      Element embedContentContainer) {
  ...
  private void handleUnknownAttachmentTypes(Document document, Message.Attachment attach,
      Element attachmentsDiv) {
  ...
  private void handleMessageReferences(GuildChannel channel, Document document, Message message,
  ...
  public FileUpload createTranscript(GuildMessageChannel channel, String fileName)
      throws IOException {
  ...
  public InputStream generateFromMessages(Collection<Message> messages) throws IOException {
    ...
    document.getElementsByClass("preamble__guild-icon").first()
        .attr("src", channel.getGuild().getIconUrl()); // set guild icon

    document.getElementById("transcriptTitle")
        .text("#" + channel.getName() + " | " + messages.size() + " messages"); // set title
